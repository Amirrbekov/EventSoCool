...
export default observer( function ActivityDetailedHeader({activity}: Props) {
    const {activityStore: {updateAttendance, loading, cancalActivityToggle}} = useStore();
    return (
        <Segment.Group>
            <Segment basic attached='top' style={{padding: '0'}}>
                ...
                <Segment style={activityImageTextStyle} basic>
                    <Item.Group>
                        ...
                        <p>
                            Hosted by <strong><Link to={`/profiles/${activity.host?.displayName}`}>{activity.host?.username}</Link></strong>
                        </p>
                    </Item.Group>
                </Segment>
            </Segment>
