...
export default observer(function ActivityForm() {
    ...
    useEffect(() => {
        if (id) loadActivity(id).then(activity => setActivity(new ActivityFormValues()));
    }, [id, loadActivity])
    ...
    function handleFormSubmit(activity: ActivityFormValues) {
        if (!activity.id) {
            ...
            createActivity(newActivity).then(() => router.navigate(`/activities/${activity.id}`))
        } else {
            ...
            <Formik enableReinitialize 
                ...
                {({handleSubmit, isValid, isSubmitting, dirty}) => (
                    <Form className="ui form" onSubmit={handleSubmit} autoComplete='off'>
                        ...
                        <MyDateInput placeholderText='Date' name='date' showTimeSelect dateFormat='MMMM d, yyyy h:mm aa' />
